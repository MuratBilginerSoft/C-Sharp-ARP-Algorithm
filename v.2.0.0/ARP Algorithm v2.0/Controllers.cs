...
namespace ARP_Algorithm_v2._0
{
    public class Controllers
    {
        ...
        public void SabitLokosyan2(string Tablo1Adı, string DosyaYolu)
        {
            Sorgu1 = "Select * from " + Tablo1Adı + "";
            DL.databasebağlan(DosyaYolu);
            DL.GenelVeriÇek(Sorgu1, Komut, Kayıt, Tablo3, DosyaYolu);
        }
        ...
        public void LokasyonYerleştir(Graphics ÇizimYap, PictureBox PictureBox1, DataTable Tablo1, Brush Dolgu1, Brush Dolgu2, Font Yazı1)
        {
            for (int i = 0; i < Tablo1.Rows.Count; i++)
            {
                ...
                double v3 = Convert.ToDouble(Tablo1.Rows[i]["X"]);
                double v4 = Convert.ToDouble(Tablo1.Rows[i]["Y"]);
                int v1 = Convert.ToInt32(v3);
                int v2=Convert.ToInt32(v4);
                ÇizimYap.FillEllipse(Dolgu1,v1,v2,25, 25);
                ÇizimYap.DrawString(Tablo1.Rows[i]["Lokasyon"].ToString(), Yazı1, Dolgu2, v1, v2);
            }
        ...
        double[] Latitude=new double[40];
        double[] Longitude = new double[40];
        ...
        public void MesafeHesapla(DataTable Tablo1, int[] RotaSeçim)
        {
            ...
            Array.Resize(ref Latitude, Tablo1.Rows.Count);
            Array.Resize(ref Longitude, Tablo1.Rows.Count);

            Toplam1 = 0;

            for (int i = 0; i < Tablo1.Rows.Count - 1; i++)
            {
                Deg1 = Convert.ToInt32(Convert.ToDouble(Tablo1.Rows[RotaSeçim[i]]["X"].ToString())/1);
                min = Convert.ToDouble(Tablo1.Rows[RotaSeçim[i]]["X"].ToString()) - Deg1;
                Latitude[i] = Pi * (Deg1 + 5.0 * min / 3.0) / 180.0;

                Deg2 = Convert.ToInt32(Convert.ToDouble(Tablo1.Rows[RotaSeçim[i]]["Y"].ToString()) / 1);
                min = Convert.ToDouble(Tablo1.Rows[RotaSeçim[i]]["Y"].ToString()) - Deg2;
                Longitude[i] = Pi * (Deg1 + 5.0 * min / 3.0) / 180.0;


                Deg1 = Convert.ToInt32(Convert.ToDouble(Tablo1.Rows[RotaSeçim[i+1]]["X"].ToString()) / 1);
                min = Convert.ToDouble(Tablo1.Rows[RotaSeçim[i+1]]["X"].ToString()) - Deg1;
                Latitude[i+1] = Pi * (Deg1 + 5.0 * min / 3.0) / 180.0;

                Deg2 = Convert.ToInt32(Convert.ToDouble(Tablo1.Rows[RotaSeçim[i+1]]["Y"].ToString()) / 1);
                min = Convert.ToDouble(Tablo1.Rows[RotaSeçim[i+1]]["Y"].ToString()) - Deg2;
                Longitude[i+1] = Pi * (Deg1 + 5.0 * min / 3.0) / 180.0;

                q1=Math.Cos(Longitude[i]- Longitude[i+1]);
                q2 = Math.Cos(Latitude[i] - Latitude[i + 1]);
                q3 = Math.Cos(Latitude[i] + Latitude[i + 1]);

                Mesafe1 = Convert.ToInt64(RRR * Math.Acos(0.5 * ((1.0 + q1) * q2 - (1.0 - q1) * q3)) + 1.0);
